...
func (h *Handler) RequestContract(w http.ResponseWriter, r *http.Request) {
	...
	if err := middleware.ParseRequest(r, &req); err != nil {
		middleware.WriteError(w, errors.ParseRequestFailed(negotiation.RequestContractEndpoint, err),
	...
	if err != nil {
		middleware.WriteError(w, ror.DSPControllerFailed(core.RoleConsumer, `RequestContract`, err),
			http.StatusBadRequest)
		return
	}
...
func (h *Handler) OfferContract(w http.ResponseWriter, r *http.Request) {
	...
	if err := middleware.ParseRequest(r, &req); err != nil {
		middleware.WriteError(w, errors.ParseRequestFailed(negotiation.OfferContractEndpoint, err),
			http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		middleware.WriteError(w, ror.DSPControllerFailed(core.RoleProvider, `OfferContract`, err),
			http.StatusBadRequest)
		return
	}
...
func (h *Handler) AcceptOffer(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		middleware.WriteError(w, errors.PathParamNotFound(negotiation.AcceptOfferEndpoint,
			negotiation.ParamConsumerPid), http.StatusBadRequest)
		return
	}

	if err := h.consumer.AcceptOffer(consumerPid); err != nil {
		middleware.WriteError(w, ror.DSPControllerFailed(core.RoleConsumer, `AcceptOffer`, err),
			http.StatusBadRequest)
		return
	}
...
func (h *Handler) AgreeContract(w http.ResponseWriter, r *http.Request) {
	...
	if err := middleware.ParseRequest(r, &req); err != nil {
		middleware.WriteError(w, errors.ParseRequestFailed(negotiation.AgreeContractEndpoint, err),
			http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		middleware.WriteError(w, ror.DSPControllerFailed(core.RoleProvider, `AgreeContract`, err),
			http.StatusBadRequest)
		return
	}
...
func (h *Handler) GetAgreement(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		middleware.WriteError(w, errors.PathParamNotFound(negotiation.GetAgreementEndpoint,
			negotiation.ParamConsumerPid), http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		middleware.WriteError(w, errors.StoreFailed(stores.TypeAgreement, `Agreement`, err),
			http.StatusBadRequest)
		return
	}
...
func (h *Handler) VerifyAgreement(w http.ResponseWriter, r *http.Request) {
	...
	consumerPid, ok := params[negotiation.ParamConsumerPid]
	if !ok {
		middleware.WriteError(w, errors.PathParamNotFound(negotiation.VerifyAgreementEndpoint,
			negotiation.ParamConsumerPid), http.StatusBadRequest)
		return
	}

	if err := h.consumer.VerifyAgreement(consumerPid); err != nil {
		middleware.WriteError(w, ror.DSPControllerFailed(core.RoleConsumer, `VerifyAgreement`, err),
			http.StatusBadRequest)
		return
	}
...
func (h *Handler) FinalizeContract(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		middleware.WriteError(w, errors.PathParamNotFound(negotiation.FinalizeContractEndpoint,
			negotiation.ParamProviderPid), http.StatusBadRequest)
		return
	}

	if err := h.provider.FinalizeContract(providerPid); err != nil {
		middleware.WriteError(w, ror.DSPControllerFailed(core.RoleProvider, `FinalizeContract`, err),
			http.StatusBadRequest)
		return
	}
...
func (h *Handler) TerminateContract(w http.ResponseWriter, r *http.Request) {
	...
	if err := middleware.ParseRequest(r, &req); err != nil {
		middleware.WriteError(w, errors.ParseRequestFailed(negotiation.TerminateContractEndpoint, err),
			http.StatusBadRequest)
		return
	}
	...
	if req.ProviderPid == `` && req.ConsumerPid != `` {
		if err := h.consumer.TerminateContract(req.ConsumerPid, req.Code, req.Reasons); err != nil {
			middleware.WriteError(w, ror.DSPControllerFailed(core.RoleConsumer, `TerminateContract`,
